...
class ConfigItem extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!!nextProps.currentCategory) {
      if (
        nextProps.change != "initial" &&
        nextProps.currentCategory._id === this.state.item._id
      ) {
  ...
  handleMoveUp() {
    var itemIndexInTable = this.state.table.findIndex(
      elem => elem._id === this.state.item._id
    );
...
const ConfigItemMapped = connect(
  mapStateToProps,
  mapDispatchToProps
)(ConfigItem);
