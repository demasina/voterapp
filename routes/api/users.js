      ...
      .then(isMatch => {
        if (isMatch) {
          if (user.confirmed) {
            ...
            errors.confirmed ="User email not confirmed. Please check mail and click verification link";
            console.log(`http://localhost:3000/verifyEmail?email=${user.email}&code=${user.verifyCode}`);
            return res.status(400).json(errors);
          }
        } else {
